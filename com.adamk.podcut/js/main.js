...
// --- state -------------------------------------------------------------------
var state = {
  setup: null,        // result of PC_getSetup
...
function scanSequence() {
  ...
  evalJSX('PC_getSetup()').then(function (raw) {
    var setup;
...
function applyEdit() {
  ...
  evalJSX('PC_applyEdit(' + JSON.stringify(payload) + ')').then(function (raw) {
    var res;
...
function resetEdit() {
  ...
  evalJSX('PC_resetEdit(' + JSON.stringify(payload) + ')').then(function (raw) {
    var res;
    try { res = JSON.parse(raw); } catch (e) { return log('ERROR: ' + raw); }
    if (!res.ok) return log('ERROR: ' + res.error);
