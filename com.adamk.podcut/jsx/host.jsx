/*
 * PodCut — ExtendScript host (runs inside Premiere Pro)
 *
 * Exposes three entry points called from the panel:
 *   PC_getSetup()          -> JSON describing the active sequence's tracks/clips
 *   PC_applyEdit(payload)  -> razors camera tracks at cut points and disables
 *                             every clip that isn't the chosen angle per segment
 *   PC_resetEdit(payload)  -> re-enables all clips on the camera tracks
 *
...
function PC_stringify(v) {
    ...
    if (v instanceof Array) {
        var parts = [];
        for (var i = 0; i < v.length; i++) parts.push(PC_stringify(v[i]));
        return '[' + parts.join(',') + ']';
    }
    var props = [];
    for (var k in v) {
        if (v.hasOwnProperty(k)) props.push(PC_stringify(k) + ':' + PC_stringify(v[k]));
    }
    return '{' + props.join(',') + '}';
}
...
function PC_secondsToTimecode(sec, fps, drop) {
...
// ---------------------------------------------------------------------------
// PC_getSetup — describe the active sequence for the panel
// ---------------------------------------------------------------------------
function PC_getSetup() {
    try {
        var seq = app.project.activeSequence;
        if (!seq) return PC_stringify({ ok: false, error: 'No active sequence. Open your podcast sequence first.' });

        var out = {
            ok: true,
            name: seq.name,
            fps: PC_getFps(seq),
            dropFrame: PC_isDropFrame(seq),
            videoTracks: [],
            audioTracks: []
        };
        ...
        return PC_stringify(out);
    } catch (e) {
        return PC_stringify({ ok: false, error: 'getSetup failed: ' + e.toString() });
    }
}

// ---------------------------------------------------------------------------
// PC_applyEdit — razor + disable
// payload: { camTracks: [videoTrackIdx...], segments: [{s, e, cam}...] }
//   cam is a video track index; segments are contiguous, in seconds.
// ---------------------------------------------------------------------------
function PC_applyEdit(payloadStr) {
    try {
        var payload = PC_parse(payloadStr);
        var seq = app.project.activeSequence;
        if (!seq) return PC_stringify({ ok: false, error: 'No active sequence.' });

        var fps = PC_getFps(seq);
        var drop = PC_isDropFrame(seq);

        app.enableQE();
        var qeSeq = qe.project.getActiveSequence();
        if (!qeSeq) return PC_stringify({ ok: false, error: 'QE could not access the sequence.' });
        ...
        for (t = 0; t < camTracks.length; t++) {
            ...
            for (i = 0; i < cutTimes.length; i++) {
                try {
                    qeTrack.razor(PC_secondsToTimecode(cutTimes[i], fps, drop));
                    razorCount++;
        ...
        return PC_stringify({ ok: true, razors: razorCount, disabled: disabled, enabled: enabled });
    } catch (e) {
        return PC_stringify({ ok: false, error: 'applyEdit failed: ' + e.toString() });
    }
}

// ---------------------------------------------------------------------------
// PC_resetEdit — re-enable every clip on the given video tracks
// payload: { camTracks: [videoTrackIdx...] }
// ---------------------------------------------------------------------------
function PC_resetEdit(payloadStr) {
    try {
        var payload = PC_parse(payloadStr);
        var seq = app.project.activeSequence;
        if (!seq) return PC_stringify({ ok: false, error: 'No active sequence.' });
        ...
        return PC_stringify({ ok: true, reEnabled: count });
    } catch (e) {
        return PC_stringify({ ok: false, error: 'resetEdit failed: ' + e.toString() });
    }
}
